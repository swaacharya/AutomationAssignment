...
class CreatePursuitPage(BasePage):
    def __init__(self, page):
        ...
        self.proposal_type_option_0 = self.page.get_by_text(pursuitData["proposal_type"]).nth(1)

        self.type_of_project = self.page.locator('[id^="rc_select_"]').nth(2)
        self.type_of_project_option_0 = self.page.get_by_text(pursuitData["type_of_project"]).nth(1)

        self.country_field = self.page.locator('[id^="rc_select_"]').nth(3)
        self.country_option_0 = self.page.get_by_text(pursuitData["country"]).nth(1)

        self.billing_arragement = self.page.locator('[id^="rc_select_"]').nth(4)
        self.billing_arragement_0 = self.page.get_by_text(pursuitData["billing_arrangement"]).nth(1)
        ...
        self.industry_option_0 = self.page.get_by_text(clientData["industry"]).nth(1)

        self.sector_field = self.page.locator('.dna-modal-body [id^="rc_select_"]').nth(1)
        self.sector_option_0 = self.page.get_by_text(clientData["sector"]).nth(1)
    ...
    def create_pursuit(self, client_name, name, description):
        ...
        pursuit_data = {
            ...
            "proposal_type": pursuitData["proposal_type"],
            "type_of_project": pursuitData["type_of_project"],
            "country": pursuitData["country"],
            "billing_arrangement": pursuitData["billing_arrangement"],
